    ...
    <section className="mx-auto max-w-7xl pb-4 md:px-0">
      <div>
        <div className="mx-auto max-w-3xl space-y-4 ">
          <div className="cursor-pointer border-b-2 border-gray-800 shadow-lg transition-all duration-200">
            <button
              type="button"
              className="flex w-full items-center justify-between px-4 py-5 sm:p-6"
            >
              <h1 className="flex text-lg font-semibold text-black w-full" onClick={()=>{
                if(!showAnswer){
            ...
            <div className={showAnswer && "px-4 pb-5 sm:px-6 sm:pb-6"}>
              { showAnswer && <p className="text-gray-400">
                {props.answer}
              </p>}
            </div>
          </div>
